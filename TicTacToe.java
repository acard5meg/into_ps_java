...
public class TicTacToe {
	...
	public void addMove()
	{
		...
		int userRow = kb.nextInt() - 1, userCol = kb.nextInt() - 1;
	...
	private boolean rowWinner(char[][] inpArray)
	{
		...
		{
			if ((inpArray[row][0] != '*') && (inpArray[row][0] == inpArray[row][1]) && (inpArray[row][1] == inpArray[row][2]))
			{
				winner = true;
			}
		}
	...
	private boolean colWinner(char[][] inpArray)
	{
		...
		{
			if ((inpArray[0][col] != '*') && (inpArray[0][col] == inpArray[1][col]) && (inpArray[1][col] == inpArray[2][col]))
			{
				winner = true;
			}
		}
